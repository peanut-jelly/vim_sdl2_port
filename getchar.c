...
AppendToRedobuffLit /*(str, len)*/
    ...
    char_u	*str,
    int		len
    )/* length of "str" or -1 for up to the NUL */
{
    char_u	*s = str;
    int		c;
    char_u	*start;
...
vgetc()
{
	...
	c = vgetorpeek(TRUE);
        //fnWarnf("vgetc vgetorpeek(TRUE)=%d", c);
	if (mod_mask)
	{
	    --no_mapping;
	    --allow_keys;
...
char_avail()
{
    ...
    void
vungetc( int c)	/* unget one character (can only be done once!) */
    //int		c;
{
    old_char = c;
 ...
 */
	if (typeahead_char != 0)
	{
        fnWarn("vgetorpeek typeahead_char!=0");
	    c = typeahead_char;
	    if (advance)
		typeahead_char = 0;
	}
	    ...
	    c = read_stuff(advance);
            //stuff buf seems empty
            //fnWarnf("c=read_stuff(advance)=%d", c);
            }
	if (c != NUL && !got_int)
	{
	    if (advance)
	...
	else
	{
		...
		if (got_int)
		{
                // seems not reaching here.
                //fnWarnf("vgetorpeek got_int");
		    /* flush all input */
		...
		{
                //fnWarnf("vgetorpeek typebuf.tb_len>0");
		    /*
 ...
 */
		wait_tb_len = typebuf.tb_len;
                /*
                fnWarnf("vgetorpeek typebuf.tb_len=%d", typebuf.tb_len);
                    {int tt=
                        !advance
			    ? 0
			    : ((typebuf.tb_len == 0
...
//#error feat_cmdl_info
    ...
    //fnWarnf("vgetorpeek returning %d", c);
    return c;
}
...
#define DUM_LEN MAXMAPLEN * 3 + 3
    ...
    //fnWarnf("inchar len=ui_inchar(...)=%d", len);
...
fix_input_buffer /*(buf, len, script)*/
    ...
    int		len,
    int		script
    )/* TRUE when reading from a script */
{
    int		i;
    char_u	*p = buf;
    ...
    {
#ifdef FEAT_GUI
    //fnWarnf("gui.in_use=%d", gui.in_use);
...
eval_map_expr /*(str, c)*/
    ...
    char_u	*str,
    int		c
    )/* NUL or typed character for abbreviation */
{
    char_u	*res;
    char_u	*p;
    char_u	*expr;
...
makemap /*(fd, buf)*/
    ...
    FILE	*fd,
    buf_T	*buf
    )/* buffer for local mappings or NULL */
{
    mapblock_T	*mp;
    char_u	c1, c2, c3;
    char_u	*p;
...
check_map /*(keys, mode, exact, ign_mod, abbr, mp_ptr, local_ptr)*/
    ...
    int		*local_ptr
    )/* return: buffer-local mapping or NULL */
{
    int		hash;
    int		len, minlen;
    mapblock_T	*mp;
