...
#define F2(i) \
    ...
    static void
bf_e_block(UINT32_T *p_xl, UINT32_T *p_xr)
{
 ...
 */
    void
bf_key_init(char_u *password, char_u *salt, int salt_len)
{
    int      i, j, keypos = 0;
    unsigned u;
 ...
 */
    static int
bf_check_tables(UINT32_T a_ipa[18], UINT32_T a_sbi[4][256], UINT32_T val)
    //UINT32_T a_ipa[18];
 ...
 */
    void
bf_ofb_init(char_u *iv, int iv_len)
    //char_u *iv;
    //int    iv_len;
{
    int i, mi;
 ...
 */
    void
bf_crypt_encode(char_u *from, size_t len, char_u *to)
    //char_u	*from;
    //size_t	len;
    //char_u	*to;
{
 ...
 */
    void
bf_crypt_decode(char_u *ptr, long len)
    //char_u	*ptr;
    //long	len;
{
    char_u	*p;
