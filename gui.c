...
#error fear_xfontset
    ...
    {
        fnWarn("in gui_init() init font failed");
 ...
 */
// this function is almost no use now.
// at least the text_area_x/y/w/h is no use.
    static void
gui_position_components(int total_width)
{
    int	    text_area_x;
    int	    text_area_y;
    int	    text_area_width;
    ...
    )
{
    ...
    // this function is no use now.
    // because i am not using native window.
...
//#error no fasttext
    ...
    if (aep != NULL && aep->ae_u.gui.font != NOFONT)
        {
        info_push_messagef("aep..font %d", SDL_GetTicks());
...
#  endif
		...
		if (thislen > 0)
		{
		    if (prev_wide)
                        {
                        //display_set_font(DISP_TASK_SETFONTWIDE, fontFlags);
		    ...
		    if (prev_wide)
                        {
                        //display_set_font(DISP_TASK_SETFONTNORMAL, fontFlags);
			gui_mch_set_font(font);
                        }
		    start += thislen;
		}
...
#ifdef FEAT_SIGN_ICONS
//#error feat_sign_icons
    if (draw_sign)
        {
	/* Draw the sign on top of the spaces. */
	gui_mch_drawsign(gui.row, col, gui.highlight_mask);
        info_push_messagef("draw sign t%d", SDL_GetTicks());
        }
#endif
...
if (!(row1==row2 && col1==col2))
    info_push_messagef("rdw_blk r1=%d c1=%d r2=%d c2=%d", 
            row1, col1, row2, col2);
            */
    ...
    //info_push_messagef("redw blk r=%d %d", row1, row2);
