 ...
 */
    int
open_buffer(int read_stdin, exarg_T *eap, int flags)
 ...
 */
    void
close_buffer(win_T *win, buf_T *buf, int action, int abort_if_last)
 ...
 */
    void
goto_buffer(exarg_T *eap, int start, int dir, int count)
    //exarg_T	*eap;
    //int		start;
    //int		dir;
    //int		count;
 ...
 */
    char_u *
do_bufdel(int command, char_u *arg, int addr_count, int start_bnr, 
...
# endif
	...
	if (top_file_num < 0)		/* wrap around (may cause duplicates) */
	{
	    EMSG(_("W14: Warning: List of file names overflow"));
            ...
            fnError("top_file_num wrap around to negative. (too many files opened)");
	    top_file_num = 1;
	}
 ...
 */
    static buf_T *
buflist_findname_stat(char_u *ffname, struct stat *stp)
    //char_u	*ffname;
    //struct stat	*stp;
{
#endif
 ...
 */
    int
buflist_findpat(char_u *pattern, char_u *pattern_end, int unlisted, 
 ...
 */
    int
ExpandBufnames(char_u *pat, int *num_file, char_u ***file, int options)
    //char_u	*pat;
    //int		*num_file;
    //char_u	***file;
    //int		options;
 ...
 */
    static char_u *
buflist_match(regprog_T *prog, buf_T *buf)
    //regprog_T	*prog;
    //buf_T	*buf;
{
    char_u	*match;
 ...
 */
    static char_u *
fname_match(regprog_T *prog, char_u *name)
    //regprog_T	*prog;
    //char_u	*name;
{
    char_u	*match = NULL;
 ...
 */
    static void
buflist_setfpos(buf_T *buf, win_T *win, linenr_T lnum, colnr_T col, 
        int copy_options)
    //buf_T	*buf;
    //win_T	*win;
    //linenr_T	lnum;
 ...
 */
    static wininfo_T *
find_wininfo(buf_T *buf, int skip_diff_buffer)
    //buf_T	*buf;
 ...
 */
    int
setfname(buf_T *buf, char_u *ffname, char_u *sfname, int message)
    //buf_T	*buf;
 ...
 */
    buf_T *
setaltfname(char_u *ffname, char_u *sfname, linenr_T lnum)
    //char_u	*ffname;
    //char_u	*sfname;
    //linenr_T	lnum;
{
    ...
    //char_u	*ffname;
{
    ...
    static int
otherfile_buf(buf_T *buf, char_u *ffname
#ifdef UNIX
	, struct stat *stp
#endif
	)
 ...
 */
    static int
buf_same_ino(buf_T *buf, struct stat *stp)
    //buf_T	*buf;
    //struct stat *stp;
{
 ...
 */
    int
build_stl_str_hl(win_T *wp, char_u *out, size_t outlen, 
        char_u *fmt, int use_sandbox, int fillchar, int maxwidth, 
 ...
 */
    void
get_rel_pos(win_T *wp, char_u *buf, int buflen)
    //win_T	*wp;
    //char_u	*buf;
    //int		buflen;
{
 ...
 */
    static int
append_arg_number(win_T *wp, char_u *buf, int buflen, int add_file)
    //win_T	*wp;
    //char_u	*buf;
    //int		buflen;
 ...
 */
    void
fname_expand(buf_T *buf, char_u **ffname, char_u **sfname)
    //buf_T	*buf UNUSED;
    //char_u	**ffname;
    //char_u	**sfname;
{
 ...
 */
    void
do_arg_all(int count, int forceit, int keep_tabs)
    //int	count;
    ...
    //int		flags;		/* Same as for do_modelines(). */
{
    ...
    int
read_viminfo_bufferlist(vir_T *virp, int writing)
    //vir_T	*virp;
    //int		writing;
{
    char_u	*tab;
 ...
 */
    static void
insert_sign(buf_T *buf, signlist_T *prev, signlist_T *next, 
 ...
 */
    void
buf_addsign(buf_T *buf, int id, linenr_T lnum, int typenr)
    //buf_T	*buf;		/* buffer to store sign in */
    //int		id;		/* sign ID */
    //linenr_T	lnum;		/* line number which gets the mark */
    //int		typenr;		/* typenr of sign we are adding */
{
    ...
    linenr_T
buf_change_sign_type(buf_T *buf, int markId, int typenr)
    //buf_T	*buf;		/* buffer to store sign in */
    //int		markId;		/* sign ID */
    //int		typenr;		/* typenr of sign we are adding */
{
    ...
    int
buf_getsigntype(buf_T *buf, linenr_T lnum, int type)
    //buf_T	*buf;
    //linenr_T	lnum;
    ...
    //int		id;		/* sign ID */
{
    ...
    int
buf_findsign_id(buf_T *buf, linenr_T lnum)
...
/* see if a given type of sign exists on a specific line */
    int
buf_findsigntype_id(buf_T *buf, linenr_T lnum, int typenr)
    //buf_T	*buf;		/* buffer whose sign we are searching for */
    //linenr_T	lnum;		/* line number of sign */
